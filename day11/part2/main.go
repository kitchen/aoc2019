...
func main() {
	...
	painter := day11.NewShipPainter(program, 1)
	for {
	...
	minY := 0
	maxY := 0
	for panel, _ := range painter.Panels {
		if panel.X < minX {
			minX = panel.X
		}
		if panel.X > maxX {
	...
	fmt.Printf("bounds are: (%v, %v) -> (%v, %v) or %v x %v\n", minX, minY, maxX, maxY, maxX-minX, maxY-minY)

	output := ""
	for y := minY; y <= maxY; y++ {
		for x := minX; x <= maxX; x++ {
			panel := day11.Panel{X: x, Y: y}
			color, exists := painter.Panels[panel]
			if exists {
				if color == 0 {
					output += ansi.Color("X", "black")
				} else {
					output += ansi.Color("X", "white")
				}
			} else {
				output += ansi.Color(".", "black")
			}
		}
		output += "\n"
	}
